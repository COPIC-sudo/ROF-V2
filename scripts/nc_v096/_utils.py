...
def append_blockers(out_dir: Path, rows: Iterable[dict[str, Any]]) -> None:
    ...
    if not rows:
        return
    path = out_dir / "BLOCKERS_V096.csv"
...
def detect_gpu() -> dict[str, Any]:
    rows: dict[str, Any] = {"decision": "CPU_USED", "reason": "GPU acceleration not used for endpoint design relabeling because no validated GPU parity implementation exists for this geometry path."}
    try:
        import torch
        ...
        rows.update({"torch_import": "missing_or_error", "torch_error": f"{type(exc).__name__}: {exc}"})
    for pkg in ["cupy", "numba"]:
        try:
            importlib.import_module(pkg)
            rows[f"{pkg}_import"] = "ok"
        except Exception as exc:
            rows[f"{pkg}_import"] = "missing_or_error"
            rows[f"{pkg}_error"] = f"{type(exc).__name__}: {exc}"
    return rows
